...
public class RequestHelper {
    ...
    static long lastCheck = PCS_Checkpoint.getInstance().config.getLong("lastCheck");
    private static final String COOKIE = PCS_Checkpoint.getInstance().config.getString("cookie");
    ...
    static void checkVerifications() {
        ...
        if(response.getStatus() != 200) {
            BungeeCord.getInstance().getLogger().severe("Could not check for verifications (HTTP " + response.getStatus() + ")");
            return;
        }
