...
public class PCS_Checkpoint extends Plugin implements Listener {
    ...
    public void onDisable() {
        //Overwrites potential other unsaved invocations of Configuration#set
        config = getConfig();
    ...
    public void onServerConnect(ServerConnectEvent event) {
        if(!(verified.containsKey(event.getPlayer()) || guests.contains(event.getPlayer())) && !event.getTarget().getName().equals("hub")) {
            ...
            event.getPlayer().sendMessage("§c§l>_ §7Bitte nutze §c§l/verify §7um deinen Account zu bestätigen!");
